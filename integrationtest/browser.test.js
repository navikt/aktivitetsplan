...
test('Validere startside', async t => {
    ...
    await t
        ...
        .expect(startside.btnMittMal.exists).ok()
...
test('Åpne dialog', async t => {
    ...
    await t
        ...
        .expect(dialog.side.exists).ok()
        .click(dialog.dialogTrad.nth(0))
        .expect(dialog.dialogDetaljvisning.exists).ok()
...
test('Åpne printmeny', async t => {
    ...
    await t
        ...
        .expect(Selector('.printmodal-body').visible).ok()
        .expect(Selector('.printmodal-header__printknapp').visible).ok()

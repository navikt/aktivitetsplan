...
const Filter = () => {
    ...
    return harAktivitet ? (
        <div onClick={preventCloseOnInsideClick} className="self-stretch sm:self-auto">
            ...
            {open ? (
                <div className="scroll-auto max-h-screen-h-1/2 rounded-md absolute p-4 bg-ax-bg-raised text-ax-text-neutral border border-ax-border-neutral z-10 w-96 max-h-screen-h-1/2 overflow-auto flex flex-col gap-y-4">
                    <AvtaltMedNavFilter />
                    <EtikettFilter />
                    <TiltakstatusFilter />
                    <AktivitetTypeFilter />

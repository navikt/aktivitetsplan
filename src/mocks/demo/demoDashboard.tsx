...
class DemoDashboard extends React.Component {
    endreTilstand: React.MouseEventHandler<HTMLInputElement> = (e) => {
    ...
    endreBrukerType = (value: 'eksternbruker' | 'veilederbruker') => {
                ...
                <section className="flex flex-wrap gap-4">
                    ...
                    <CheckboxGroup value={getChecked(features)} legend={'Brukers tilstand'}>
                        {features.map(({ id, label, checked }) => (
                            <Checkbox key={id} value={id} id={id} onClick={this.endreTilstand}>
                                {label}
                            </Checkbox>
                        ))}
                    </CheckboxGroup>
                    <CheckboxGroup value={getChecked(aktivitetTilstand)} legend={'Aktivitet tilstand'}>
                        {aktivitetTilstand.map(({ id, label, checked }) => (
                            <Checkbox key={id} value={id} id={id} onClick={this.endreTilstand}>
                                {label}
                            </Checkbox>
                        ))}
                    ...
                    <CheckboxGroup value={getChecked(feiltilstander)} legend="Feiltilstander">
                        {feiltilstander.map(({ id, label, checked }) => (
                            <Checkbox key={id} value={id} id={id} onClick={this.endreTilstand}>
                                {label}
                            </Checkbox>
                        ))}

...
const featureQueryParams = features => {
    const reduceFunc = (acc, toggle, i) => `${acc}${i === 0 ? '?' : '&'}feature=${toggle}`;
...
export function hentFeature(enhet) {
    ...
    return fetchToJson(`${window.appconfig.CONTEXT_PATH}/api/feature${featuresWithEnhet}`).catch(e => console.log(e));
}
